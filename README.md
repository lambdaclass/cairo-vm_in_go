...
This method will relocate the memory and trace generated by the program execution. Relocating means that our VM transforms a bidimensional memory (aka a memory divided by segments) to a continuous, one-dimensional memory. The memory relocation process is explained at a high level [here](#memory-relocation).
...
Notice that each step is encapsulated into a function. We will look into each one in the following subsections.
